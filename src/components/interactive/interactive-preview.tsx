...
export function InteractivePreview({
  ...
  const tx = useTranslations("XInteractiveInteractivePreview");
  return (
    <div
      className={cn(
    ...
    >
      <div className="px-3 py-2">
        ...
        <p className="whitespace-pre-wrap break-words text-sm">
          {payload.body || (
            <span className="text-muted-foreground">{tx("bodyPlaceholder")}</span>
          )}
        </p>
        {payload.footer ? (
      ...
      {payload.kind === "buttons" ? (
        <div className="flex flex-col border-t border-border">
          {payload.buttons.map((b, i) => (
            ...
            >
              ...
              <span className="truncate">{b.title || tx("button")}</span>
            </button>
          ))}
        </div>
      ) : (
        ...
        >
          ...
          <span className="truncate">{payload.button_label || tx("menu")}</span>
        </button>
      )}
    </div>
  );
