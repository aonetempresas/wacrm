...
export default function JoinPage() {
  ...
  const tx = useTranslations('XJoinToken');

  const ROLE_LABEL: Record<PeekOk['role'], string> = {
    admin: tx('roleAdmin'),
    agent: tx('roleAgent'),
    viewer: tx('roleViewer'),
  };

  const FAIL_COPY: Record<PeekFail['reason'], { title: string; body: string }> = {
    not_found: { title: tx('failNotFoundTitle'), body: tx('failNotFoundBody') },
    used: { title: tx('failUsedTitle'), body: tx('failUsedBody') },
    expired: { title: tx('failExpiredTitle'), body: tx('failExpiredBody') },
    server_error: {
      title: tx('failServerErrorTitle'),
      body: tx('failServerErrorBody'),
    },
  };
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (res.status === 409) {
          setConflictMessage(payload.error || tx('toastAlreadyInAccount'));
        } else {
          toast.error(payload.error || tx('toastFailedAccept'));
        }
        setAccepting(false);
        return;
      }
      toast.success(tx('toastWelcome'));
      // Full reload (not router.push) so AuthProvider re-fetches
      // the profile with the new account_id and account_role.
      window.location.href = '/dashboard';
    } catch (err) {
      console.error('[join] redeem error:', err);
      toast.error(tx('toastCouldNotReach'));
      setAccepting(false);
    }
  }, [token]);
  ...
  const handleSignOutAndRetry = useCallback(async () => {
    ...
    try {
      ...
      toast.error(tx('toastCouldNotSignOut'));
      setSigningOut(false);
    }
  }, []);
    ...
    return (
      <Card className="w-full max-w-md border-border bg-card">
        <CardContent className="flex flex-col items-center gap-3 py-12">
          ...
          <p className="text-sm text-muted-foreground">{tx('verifying')}</p>
        </CardContent>
      </Card>
    );
  }
          ...
          <DialogContent className="bg-popover border-border sm:max-w-md">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2 text-popover-foreground">
                ...
                {tx('cantJoin', { name: peek.account_name })}
              </DialogTitle>
