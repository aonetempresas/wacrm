...
- **2026-07-24** — **Ajustes de UX do Painel (feedback do Renan): presets do filtro + Atividade recente.** Presets do período trocados para janelas móveis consistentes: **Últimos 7 dias · Últimos 30 dias · Último ano · Mês passado** (o "30 dias" vs "mês" confundia); calendário De/Até movido pra frente; default = Últimos 30 dias (`defaultPeriod`). **Atividade recente** repensada: (1) removido o "Ver tudo →" que apontava errado pra Caixa de entrada; (2) removida a paginação "Mostrar 5/10/20"; (3) **textos traduzidos pt-BR** — `ActivityItem` virou estruturado (kind/subject/context/count/variant), `loadActivity` monta os dados e o componente traduz via i18n (`Dashboard.activityFeed` msgFrom/newContact/dealInStage/etc); (4) **agrupado por DIA, cada dia recolhível ao clicar na palavra** (Hoje aberto, resto fechado; dias antigos mostram a data real em vez de "Anteriores" vago). Validado (tsc/build/i18n 1973=1973). **Não commitado ainda.**
- **2026-07-24** — **Filtro de período global no Painel (a "espinha dorsal" do cockpit).** Componente `period-filter.tsx` (presets Este mês/Mês passado/30 dias/Este ano + calendário De/Até) no topo do Painel. `loadSalesResults` e `loadRepPerformance` agora recebem (from,to) e filtram ganho/perdido por won_at/lost_at no intervalo (antes usava updated_at/monthStart — mais preciso agora). O Painel separa carregamento período-independente (loadAll: gráfico conversas, donut, tempo resposta, atividade) do período-dependente (loadResults: Zona 1 + por consultor), que re-roda quando o período muda → tudo sincronizado pro mesmo intervalo, permitindo comparar períodos. "Em negociação" (pipeline aberto) fica sempre atual (não é dado de período). Labels "mês" → "período" onde virou genérico. Franqueza pro Renan: o filtro rege as métricas de "fechado no período"; o gráfico de conversas ainda é fixo em 30 dias (widget de atendimento, não migrado). Validado (tsc/build/i18n 1960=1960). **Próximo (ordem acordada):** gráfico de evolução → painel de atividade do time (agenda no Painel) → 2B metas/farol. Não commitado ainda.
- **2026-07-24** — **Etapa 2A: cockpit do diretor (Painel de resultados).** O Painel deixou de mostrar métricas de atendimento no topo (conversas ativas, mensagens enviadas — o diretor não liga) e virou leitura de resultados. **Zona 1** (topo, 4 cards de resultado): Em negociação (R$ pipeline aberto), Ganhos no mês (R$ + nº vendas), Conversão (%), Ticket médio. **Zona 2** (gestão, admin+): Desempenho por consultor (já existia) + **Vendas por canal** (de onde vêm as vendas) + **Motivos de perda** (ranking do "por que perdemos"), ambos com barras. Nova query `loadSalesResults` (agrega deals: aberto/ganho-no-mês/perdido-no-mês por won_at/lost_at, canal, motivos). Tipo `SalesResults`, componente `results-panels.tsx`, i18n `Dashboard.results` + `Dashboard.page` (inNegotiation/wonThisMonth/wonCount/conversion/avgTicket). Removidos loadMetrics/MetricsBundle/deltaLabel do Painel (métricas de atendimento operacional saíram do topo; gráfico de conversas + tempo de resposta continuam mais abaixo). Franqueza: conversão = ganhos÷(ganhos+perdidos) do mês (não a estrita do briefing). Validado (tsc/build/i18n 1955=1955). **Falta:** Zona 3 (metas+projeção+farol = 2B) e commit do 2A.
- **2026-07-23** — **Cores da temperatura + alertas animados no card (ideia do Renan).** Temperatura recolorida pra ficar distinta: **Quente=vermelho, Morno=amarelo, Frio=azul** (antes quente=laranja/morno=âmbar confundiam). No card do funil: 🔥 **fogo pulsando** (animate-bounce) quando o lead é quente, e ⚠️ **triângulo piscando** (animate-pulse) quando o negócio aberto está **parado há STUCK_DAYS (7) dias** sem movimento. `STUCK_DAYS` centralizado em `aonet-lists.ts` (usado no card e no top-card "Parados"). Limiar de 7 dias é ajustável (Renan sugeriu 24h; ficou em 7 por ser menos ruidoso — a combinar). Validado (tsc/build/i18n 1940=1940). Card + cores + item 3 (topo enxuto) commitados.
- **2026-07-23** — **Etapa 1.3: topo do funil enxuto.** `pipeline-analytics.tsx` reescrito: saíram os 6 cards genéricos (Total, Valor do funil, Tamanho médio, Valor ponderado, Ganhos/Perdidos no mês) e entraram 4 operacionais → **Negócios abertos · Em negociação (R$) · 🔥 Quentes · ⏳ Parados (7+ dias)**. Só de deals abertos, foco no operador; métricas de gestor ficam pro cockpit do Painel. Prop `stages` removido do componente. Validado (tsc/build/i18n 1939=1939). **Etapa 1 (Pacote do Funil) COMPLETA.** Item 3 ainda não commitado. Próximo: **Etapa 2 — cockpit do diretor**.
- **2026-07-23** — **Etapa 1.2: Ganho sai do funil → aba "Ganhos".** Migração `040_deal_close_dates.sql` (deals: won_at, lost_at) — rodada pelo Renan. `deal-form.tsx` carimba won_at/lost_at ao marcar Ganho/Perdido (limpa no reabrir). Página do Funil ganhou toggle **"Funil | Ganhos"**: o quadro mostra só negócios EM ABERTO (won/lost saem do board — resolve a confusão do card ganho "preso" no funil que o Renan reportou); investigado e confirmado que NÃO havia bug de "um clique marca dois" (os 2 ganhos de teste foram 2 ações separadas, 12s de diferença — resetados pra open). Componente `closed-deals-view.tsx` genérico (mode won/lost) serve **Ganhos E Perdidos**: lista com **filtro por calendário (De/Até, `input type=date`)**, total, "Fechado/Perdido em {data}"; a aba Perdidos mostra também os **motivos da perda** (chips). Toggle no topo do Funil: **Funil · Ganhos · Perdidos** (sem badge de contagem — a pedido do Renan). Ganho E Perdido saem do board. Validado (tsc/build/i18n 1931=1931). Card + migração 040 + Ganhos/Perdidos commitados.
- **2026-07-23** — **Reorganização do rumo (alinhada com o Renan) + Fase C1 + card do funil.** C1 "Leads esquecidos" na Agenda commitada (`823ba33`). Decisão: reprioritizar o roadmap pra virar em direção ao **"cockpit do diretor"** (Painel de resultados p/ gestão) — pausa C2/C3/D. Nova ordem: **Etapa 1 — Pacote do Funil** (1. card mais limpo ✅ / 2. Ganho sai do funil → aba "Ganhos" dentro do Funil, com data de fechamento / 3. cards do topo do funil enxutos e operacionais) → **Etapa 2 — Cockpit do diretor** (Painel em 3 zonas: agora / o que houve / o que vai acontecer = itens 6.2 + 11 do briefing: conversão, ticket, motivos de perda, por consultor, por canal, MRR, meta+projeção+farol). Por quê essa ordem: o cockpit LÊ os ganhos (com data) e o pipeline — a Etapa 1 destrava a Etapa 2. **Card redesenhado** (`deal-card.tsx`): empresa em destaque (B2B), contato+produto como apoio, valor, temperatura pinta o card — sem a redundância de nome. 6 leads de exemplo criados via script (contatos + negócios + tarefas) p/ dar "vida real". **Decisões pendentes:** "Ganho sai do funil pra aba Ganhos" (não colorir contato — won é do negócio, não do contato); aba Ganhos = **dentro do Funil** (recomendado, não item da barra); precisa coluna `won_at`/`closed_at` no deal p/ o filtro de data. **Card não commitado ainda.**
- **2026-07-23** — **Enriquecimento do lead (contato) + remoção do "Campos personalizados" genérico.** Migração `039_contact_market_intel.sql` (colunas aditivas em `contacts`: city, current_operator, current_monthly_price, pain_points[], pain_note) — rodada pelo Renan. Constantes L5 (operadoras) e L7 (dores) em `aonet-lists.ts`. Seção **"Inteligência de mercado (opcional)"** recolhida no formulário de contato (`contact-form.tsx`) — criar E editar — com Cidade, Operadora atual (lista), Valor que paga hoje (R$), Necessidade/dor (multi), abre sozinha se o contato já tiver dados. **Recurso genérico "Campos personalizados" REMOVIDO da vista** (a pedido do Renan, que tropeçava nele 3x): tirado o botão no topo de Contatos e a aba "Campos" da ficha (com todo o código relacionado — state/fetch/save/imports). O recurso/DB continua existindo, só não aparece; bônus: 1 aba a menos na ficha. Validado (tsc/build/i18n 1911=1911). **Commitado e no GitHub: `7e76688`.** Nota consciente: esses campos ainda NÃO aparecem como coluna na lista de contatos nem em relatório — próximo passo natural.
